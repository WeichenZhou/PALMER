...
//int main(){
    ...
    for(int i=0;i!=line_tsd;i++) loc_tsd[i]=new int[11];
    ...
    */
    ...
    for(int i=0;i!=line;i++){
        if(loc[i][6]!=-1){
            ...
            seq_index=info[i][0]+"."+loc_0.c_str()+"."+loc_1.c_str()+"."+loc_2.c_str()+"."+loc_3.c_str()+"."+loc_4.c_str()+"."+loc_5.c_str()+"."+info[i][1]+"."+orien[i];
                    ...
                    if(info[i][0]==info[j][0]&&loc[j][6]==0&&info[i][1]==info[j][1]&&loc[i][0]==loc[j][0]&&loc[i][1]==loc[j][1]&&loc[i][2]==loc[j][2]&&loc[i][3]==loc[j][3]&&loc[i][4]==loc[j][4]&&loc[i][5]==loc[j][5]&&orien[i]==orien[j]){
                        ...
                        if((loc[j][4]>=start1&&loc[j][4]<=start2)||(loc[j][5]<=end2&&loc[j][5]>=end1)){
                            ...
                            if((loc[j][0]>=L1_s1&&loc[j][0]<=L1_s2)&&(loc[j][1]>=L1_e1&&loc[j][1]<=L1_e2)){
                                ...
                                seq_index=info[j][0]+"."+loc_0.c_str()+"."+loc_1.c_str()+"."+loc_2.c_str()+"."+loc_3.c_str()+"."+loc_4.c_str()+"."+loc_5.c_str()+"."+info[j][1]+"."+orien[j];
                ...
                if((loc[j][0]>=L1_s1&&loc[j][0]<=L1_s2)&&(loc[j][1]>=L1_e1&&loc[j][1]<=L1_e2)&&(loc[j][4]>=start1&&loc[j][4]<=start2)&&(loc[j][5]>=end1&&loc[j][5]<=end2)&&loc[j][6]==-1&&info[i][1]==info[j][1]&&orien[i]==orien[j]){
                    ...
                    for(int w=0;w!=line_tsd;w++){
                        if(info_tsd[w]==seq_index&&loc_tsd[w][8]==1){
                            loc_tsd[w][4]=1;
                            flag_number=1;
                            if(loc_tsd[w][9]==0&&loc_tsd[w][10]==0){
                                flag_true_number=1;
                            }
                        }
                    }
            ...
            //loc_tsd[][5]
            for(int w=0;w!=line_tsd;w++){
                if(loc_tsd[w][4]==1&&loc_tsd[w][9]==0&&loc_tsd[w][10]==0){
                    loc_tsd[w][5]=1;
            ...
            for(;flag_ployA==0&&flag_trans==1;){
                ...
                for(int w2=0;w2!=line_tsd;w2++){
                    if(loc_tsd[w2][4]==1&&loc_tsd[w2][5]==p&&loc_tsd[w2][9]==0&&loc_tsd[w2][10]==0){
                        if(orien[i]=="+"){
                ...
                if(flag_trans==0){
                    ...
                    //cluster output
                    file2<<"cluster"<<i<<"_"<<info[i][1]<<"_"<<start1+S<<"_"<<start2-S<<"_"<<end1+S<<"_"<<end2-S<<'\t'<<info[i][1]<<'\t'<<start1+S<<'\t'<<start2-S<<'\t'<<end1+S<<'\t'<<end2-S<<'\t'<<L1_s1+L<<'\t'<<L1_s2-L<<'\t'<<L1_e1+L<<'\t'<<L1_e2-L<<'\t'<<number_true<<'\t'<<p<<'\t'<<(number_all-number)<<'\t'<<orien[i]<<'\t'<<l_len<<'\t'<<r_len<<'\t'<<trans_len<<endl;
